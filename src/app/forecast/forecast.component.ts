...
export class ForecastComponent implements OnInit {
  ...
  filterWeather(){
    ...
    for(let i = 0; i<5; i++){
      ...
      let today = now.toISOString().split('T')[0];
      //this.weatherArray.push(this.weather.list.filter((w: any) => w.dt_txt == today + " 18:00:00")[0]); mozna pomyslec z includem ale to da pierwszy z brzegu
      this.weatherArray.push(this.weather.list.filter((w: any) => w.dt_txt == today + " 21:00:00")[0]);
    }
